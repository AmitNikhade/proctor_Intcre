...
class VideoTransformTrack(VideoStreamTrack):
    ...
    async def recv(self):
        ...
        cv2.imshow('Outgoing Video - Client 2', frame)
...
async def run_answer(pc, offer):
    ...
    logger.debug(f"Remote Description set with SDP: {offer['sdp']}")
    await pc.setLocalDescription(await pc.createAnswer())
    logger.debug(f"Local Description (Answer): {pc.localDescription.sdp}")
...
async def consume_signaling(pc, websocket):
    async for message in websocket:
        ...
        if msg["type"] == "offer":
            ...
            logger.info(f"Sent answer: {answer}")
        else:
            logger.warning(f"Unexpected message type: {msg['type']}")

async def main():
    recorder = MediaRecorder("received_video_client2.mp4")
        ...
        if track.kind == "video":
            ...
            async def display_incoming_video():
                while True:
                    ...
                    cv2.imshow('Incoming Video - Client 2', img)
                    if cv2.waitKey(1) & 0xFF == ord('q'):
                        raise RuntimeError("Quitting video display")
